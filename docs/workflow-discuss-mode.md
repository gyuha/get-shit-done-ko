...
|------|----------------|-------------------|
| `--auto` | 추천 답변을 자동 선택 | 확인 단계를 건너뛰고 `Unclear` 항목을 자동 해소 |
| `--batch` | 질문을 묶음으로 그룹화 | 해당 없음(수정 항목이 이미 묶여 있음) |
| `--text` | 일반 텍스트 질문(원격 세션) | 일반 텍스트 질문(원격 세션) |
| `--analyze` | 질문별 트레이드오프 표를 표시 | 해당 없음(assumptions에 근거가 포함됨) |
...
이후 단계의 researcher, planner, checker는 어떤 모드에서 만들었든 동일한 방식으로 이 CONTEXT.md를 사용합니다.
