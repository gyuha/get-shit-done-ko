...
localization audit 단계에서는 `node scripts/audit-localization-gap.cjs --to-tag <latest_tag> --mode source-of-truth --json`를 실행해 `changed_files`, `overlay_reapply`, `overlay_delete`, `translation_candidates`, `overlay_missing`, `zh_cn_reintroduced`, `token_sensitive_candidates`를 확인합니다.
...
`token_sensitive_candidates`는 commands, file paths, placeholders, `@` references, XML/markdown structure, identifiers 검증이 필요한 고위험 항목입니다.
